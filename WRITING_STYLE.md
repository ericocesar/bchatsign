...
This document defines the writing conventions for Bchatsign documentation.
...
1. **Users** - People using the Bchatsign web application to send and sign documents
2. **Developers** - Building integrations with the API or SDKs
3. **Self-hosters** - Running their own Bchatsign instance
...
```typescript
// Basic usage
const document = await bchatsign.documents.create({
  title: "Contract",
  file: pdfBuffer,
});
