...
test('[TEMPLATE]: should create a document from a template', async ({ page }) => {
  ...
  await page.getByPlaceholder('Email').nth(1).fill('recipient2@bchatsign.com');
  ...
  expect(document.documentMeta?.redirectUrl).toEqual('https://bchatsign.com');
...
test('[TEMPLATE]: should create a document from a template with custom document', async ({ page }) => {
  ...
  // Add a signer
  await page.getByPlaceholder('Email').fill('recipient@bchatsign.com');
