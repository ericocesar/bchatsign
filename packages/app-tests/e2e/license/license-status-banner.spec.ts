...
const LICENSE_FILE_NAME = '.bchatsign-license.json';
const LICENSE_BACKUP_FILE_NAME = '.bchatsign-license-backup.json';
...
const getLicenseFilePath = () => {
  // From packages/app-tests/e2e/license -> ../../../../apps/remix/.bchatsign-license.json
...
test.describe
  .skip('License Status Banner', () => {
    ...
    test('[ADMIN]: no banner when license file is missing', async ({ page }) => {
      ...
      await expect(page.getByText('This is an expired license instance of Bchatsign')).not.toBeVisible();
    ...
    test.skip('[ADMIN]: global banner shows when EXPIRED with unauthorized usage', async ({ page }) => {
      ...
      await expect(page.getByText('This is an expired license instance of Bchatsign')).toBeVisible();
      ...
      await expect(
        page.getByText('Your Bchatsign instance is using features that are not part of your license.'),
      ).toBeVisible();
    });
