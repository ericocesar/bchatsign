...
import { createEmbeddingPresignToken } from '@bchatsign/lib/server-only/embedding-presign/create-embedding-presign-token';
...
} from '@bchatsign/prisma/seed/documents';
import { seedBlankTemplate, seedDirectTemplate } from '@bchatsign/prisma/seed/templates';
...
test.describe('PDF Viewer Rendering', () => {
  ...
  test.describe('Recipient Signing', () => {
    test('should render PDF on signing page (V1 and V2)', async ({ page }) => {
      ...
      const { recipients: recipientsV1 } = await seedPendingDocumentWithFullFields({
        owner: user,
        teamId: team.id,
        recipients: ['signer-v1@test.bchatsign.com'],
      ...
      const { document: documentV2, recipients: recipientsV2 } = await seedPendingDocumentWithFullFields({
        owner: user,
        teamId: team.id,
        recipients: ['signer-v2@test.bchatsign.com'],
        fields: [FieldType.SIGNATURE],
  ...
  test.describe('Share Page', () => {
    test('should render PDF on share page (V1 and V2)', async ({ page }) => {
      ...
      const documentV1 = await seedCompletedDocument(user, team.id, ['share-v1@test.bchatsign.com'], {
        createDocumentOptions: { qrToken: qrTokenV1 },
      });

      const documentV2 = await seedCompletedDocument(user, team.id, ['share-v2@test.bchatsign.com'], {
  ...
  test.describe('Embed Pages', () => {
    test('should render PDF on embed sign page (V1 and V2)', async ({ page }) => {
      ...
      const { recipients: recipientsV1 } = await seedPendingDocumentWithFullFields({
        owner: user,
        teamId: team.id,
        recipients: ['embed-signer-v1@test.bchatsign.com'],
        fields: [FieldType.SIGNATURE],
      });
      ...
      const { document: documentV2, recipients: recipientsV2 } = await seedPendingDocumentWithFullFields({
        owner: user,
        teamId: team.id,
        recipients: ['embed-signer-v2@test.bchatsign.com'],
        fields: [FieldType.SIGNATURE],
        updateDocumentOptions: { internalVersion: 2 },
      });
      await addSecondEnvelopeItem(documentV2.id);
    ...
    test('should render PDF on embed multisign page (V1 and V2)', async ({ page }) => {
      ...
      const { recipients: recipientsV1 } = await seedPendingDocumentWithFullFields({
        owner: user,
        teamId: team.id,
        recipients: ['multisign-v1@test.bchatsign.com'],
        fields: [FieldType.SIGNATURE],
      });
      ...
      const { document: documentV2, recipients: recipientsV2 } = await seedPendingDocumentWithFullFields({
        owner: user,
        teamId: team.id,
        recipients: ['multisign-v2@test.bchatsign.com'],
        fields: [FieldType.SIGNATURE],
        updateDocumentOptions: { internalVersion: 2 },
      });
      await addSecondEnvelopeItem(documentV2.id);
    ...
    test('should render PDF on embed authoring document create page', async ({ page }) => {
      ...
      await emailInput.fill('test-signer@bchatsign.com');
