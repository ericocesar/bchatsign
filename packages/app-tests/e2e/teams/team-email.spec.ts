...
import { seedTeamEmailVerification } from '@bchatsign/prisma/seed/teams';
...
test('[TEAMS]: send team email request', async ({ page }) => {
  ...
  await page.getByPlaceholder('eg. Legal').fill('test@test.bchatsign.com');
  await page.getByPlaceholder('example@example.com').click();
  await page.getByPlaceholder('example@example.com').fill('test@test.bchatsign.com');
...
test('[TEAMS]: accept team email request', async ({ page }) => {
  ...
  const teamEmailVerification = await seedTeamEmailVerification({
    email: `team-email-verification--${team.url}@test.bchatsign.com`,
    teamId: team.id,
  });
