 ...
 * CSS nesting (`.bchatsign-branded { <user css> }`). The sanitiser itself
 ...
 */
export const SANITIZE_BRANDING_SCOPE_CLASS = 'bchatsign-branded';
 ...
 * The sanitised output is later wrapped in `.bchatsign-branded { ... }` via
...
const sanitizeRule = (rule: Rule, warnings: SanitizeBrandingCssWarning[]): void => {
  ...
  // the entire sanitised CSS in `.bchatsign-branded { ... }` (CSS nesting).
