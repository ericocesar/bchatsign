...
const HAS_LICENSE_KEY = !!process.env.NEXT_PRIVATE_BCHATSIGN_LICENSE_KEY;

const TELEMETRY_KEY = process.env.NEXT_PRIVATE_TELEMETRY_KEY;
const TELEMETRY_HOST = process.env.NEXT_PRIVATE_TELEMETRY_HOST;
const TELEMETRY_DISABLED = !!process.env.BCHATSIGN_DISABLE_TELEMETRY || HAS_LICENSE_KEY;

const NODE_ID_FILENAME = '.bchatsign-node-id';
...
export class TelemetryClient {
   ...
   *
   * If telemetry is disabled via `BCHATSIGN_DISABLE_TELEMETRY=true` or credentials are not
  ...
  public static async start(): Promise<void> {
    if (TELEMETRY_DISABLED) {
      if (!HAS_LICENSE_KEY) {
        console.log(
          '[Telemetry] Telemetry is disabled. To enable, remove the BCHATSIGN_DISABLE_TELEMETRY environment variable.',
        );
      }
  ...
  private async initialize(): Promise<void> {
    ...
    console.log(
      '[Telemetry] Telemetry is enabled. Bchatsign collects anonymous usage data to help improve the product.',
    );
    console.log(
    ...
    console.log(
      '[Telemetry] To disable telemetry, set BCHATSIGN_DISABLE_TELEMETRY=true in your environment variables.',
    );
    console.log('[Telemetry] Learn more: https://bchatsign.com/docs/developers/self-hosting/telemetry');
