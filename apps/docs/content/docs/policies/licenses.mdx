---
title: Licenses
description: Bchatsign uses a dual licensing model with an open-source AGPL-3.0 license (Community Edition) and a commercial license (Enterprise Edition).
---
...
- Use Bchatsign for any purpose, including commercial use
- Access, study, and modify the source code
- Distribute copies of the original or modified software
- Self-host Bchatsign on your own infrastructure

### What AGPL-3.0 Requires

The AGPL-3.0 has specific requirements that apply when you modify or distribute Bchatsign:

**Source Code Disclosure**: If you modify Bchatsign and make it available over a network, you must provide access to the complete source code of your modified version under AGPL-3.0.
...
<Callout type="info">
  ...
  network access as distribution. If users interact with your modified Bchatsign instance over a
...
- Use Bchatsign internally without modifications
...
The Enterprise Edition provides a commercial license that removes the AGPL-3.0 requirements. This allows organisations to use, modify, and integrate Bchatsign without source code disclosure obligations.
...
**Proprietary Integration**: Integrate Bchatsign into proprietary products and services without licensing conflicts.
...
- Build commercial products that incorporate Bchatsign
...
<Callout type="warn">
  If you modify Bchatsign and provide network access to users (internal or external) without
...
<Accordions type="multiple">
  <Accordion title="Do you modify Bchatsign?">
    **No modifications**: Both licenses work. Community Edition is free and sufficient if you run Bchatsign as-is.
  ...
  <Accordion title="Are you embedding Bchatsign in a commercial product?">
  ...
  <Accordion title="Internal use only?">
    Using Bchatsign internally without providing network access to external users does not trigger AGPL-3.0 distribution requirements. However, if your organisation has policies against AGPL software or you make modifications you want to keep private, consider the Enterprise Edition.
  </Accordion>
</Accordions>
...
<Accordions type="multiple">
  ...
  <Accordion title="Does using the Bchatsign API require a commercial license?">
    No. Calling the Bchatsign API from your application does not create a derivative work. Your
    application remains under your chosen license. However, if you modify Bchatsign itself and
    provide network access, AGPL-3.0 requirements apply.
  </Accordion>
  <Accordion title='What counts as a "modification" under AGPL-3.0?'>
    Modifying the source code, forking the repository and making changes, or linking Bchatsign
    libraries into your application in a way that creates a combined work. Configuration changes and
    using Bchatsign through its API are not modifications.
  </Accordion>
  <Accordion title="Can I white-label Bchatsign?">
  ...
  <Accordion title="Can I contribute to Bchatsign under the Enterprise license?">
    Contributions to the main Bchatsign repository are licensed under AGPL-3.0. The Enterprise
...
- [GitHub Repository](https://github.com/bchatsign/bchatsign) - Source code and LICENSE file
