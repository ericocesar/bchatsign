...
description: Deploy Bchatsign with Docker Compose, including PostgreSQL. This production-ready setup is suitable for most self-hosted deployments.
---
...
```bash
mkdir bchatsign && cd bchatsign
curl -O https://raw.githubusercontent.com/bchatsign/bchatsign/release/docker/production/compose.yml
```

Alternatively, clone the full repository:

```bash
git clone https://github.com/bchatsign/bchatsign.git
cd bchatsign/docker/production
```
...
services:
  ...
  bchatsign:
    ...
    ports:
      - ${PORT:-3000}:${PORT:-3000}
    volumes:
      - /opt/bchatsign/cert.p12:/opt/bchatsign/cert.p12:ro
...
| `bchatsign` | Main application container, waits for database to be healthy |

The Bchatsign container waits for the database health check to pass before starting.
...
```bash
# Database (used by both database and bchatsign services)
...
# Public URL where Bchatsign is accessible
NEXT_PUBLIC_WEBAPP_URL=https://sign.example.com
NEXT_PRIVATE_INTERNAL_WEBAPP_URL=http://localhost:3000

# Database connection (uses Docker service name)
NEXT_PRIVATE_DATABASE_URL=postgresql://bchatsign:your-secure-database-password@database:5432/bchatsign
...
```bash
sudo mkdir -p /opt/bchatsign
sudo cp /path/to/your/cert.p12 /opt/bchatsign/cert.p12
sudo chown 1001:1001 /opt/bchatsign/cert.p12
sudo chmod 400 /opt/bchatsign/cert.p12
```
...
```
NAME                             STATUS              PORTS
bchatsign-production-database-1  running (healthy)   5432/tcp
bchatsign-production-bchatsign-1 running             0.0.0.0:3000->3000/tcp
```
...
```bash
docker compose --env-file .env restart bchatsign
```
...
For production, place Bchatsign behind a reverse proxy for SSL termination:
...
```bash
# Manual backup
docker compose exec database pg_dump -U bchatsign bchatsign > backup.sql

# Restore from backup
docker compose exec -T database psql -U bchatsign bchatsign < backup.sql
```
...
```bash
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@your-db-host:5432/bchatsign
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@your-db-host:5432/bchatsign
```
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check logs: `docker compose logs bchatsign`. Common causes: missing environment variables
  ...
  <Accordion title="Database connection errors">
    ...
    database psql -U bchatsign -d bchatsign -c "SELECT 1"`.
  </Accordion>
  ...
  <Accordion title="Emails not sending">
    ...
    password, check logs: `docker compose logs bchatsign | grep -i smtp`.
  </Accordion>
