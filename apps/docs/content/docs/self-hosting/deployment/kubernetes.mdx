---
title: Kubernetes
description: Deploy Bchatsign on Kubernetes for production environments requiring high availability, auto-scaling, and enterprise-grade infrastructure.
---
...
A typical Bchatsign Kubernetes deployment consists of:
...
Create a dedicated namespace for Bchatsign:
...
metadata:
  name: bchatsign
  labels:
    app.kubernetes.io/name: bchatsign
```
...
stringData:
  ...
  NEXT_PRIVATE_DATABASE_URL: 'postgresql://user:password@postgres-host:5432/bchatsign'
  NEXT_PRIVATE_DIRECT_DATABASE_URL: 'postgresql://user:password@postgres-host:5432/bchatsign'
...
data:
  ...
  NEXT_PRIVATE_SMTP_FROM_NAME: 'Bchatsign'
  NEXT_PRIVATE_SMTP_FROM_ADDRESS: 'noreply@example.com'
  NEXT_PRIVATE_INTERNAL_WEBAPP_URL: 'http://localhost:3000'
  NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH: '/opt/bchatsign/cert.p12'
...
Create the Bchatsign Deployment:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: bchatsign
          image: bchatsign/bchatsign:latest
          ...
          volumeMounts:
            - name: signing-cert
              mountPath: /opt/bchatsign/cert.p12
              subPath: cert.p12
              readOnly: true
      volumes:
        - name: signing-cert
          secret:
            secretName: bchatsign-signing-cert
            items:
              - key: cert.p12
                path: cert.p12
```
...
<Callout type="info">
  Pin to a specific image tag (e.g., `bchatsign/bchatsign:1.5.0`) in production instead of `latest`
...
```bash
kubectl create secret generic bchatsign-signing-cert \
  --namespace bchatsign \
...
type: Opaque
stringData:
  POSTGRES_USER: bchatsign
  POSTGRES_PASSWORD: your-secure-password
  POSTGRES_DB: bchatsign
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
Update your Bchatsign secret to use the in-cluster database:

```yaml
NEXT_PRIVATE_DATABASE_URL: 'postgresql://bchatsign:your-secure-password@postgres.bchatsign.svc.cluster.local:5432/bchatsign'
NEXT_PRIVATE_DIRECT_DATABASE_URL: 'postgresql://bchatsign:your-secure-password@postgres.bchatsign.svc.cluster.local:5432/bchatsign'
```

## Persistent Storage

Bchatsign stores documents in the database by default. For high-volume deployments, configure S3-compatible storage.
...
stringData:
  ...
  NEXT_PRIVATE_UPLOAD_BUCKET: 'your-bchatsign-bucket'
...
Scale Bchatsign based on CPU and memory utilization:
...
<Accordions type="multiple">
  <Accordion title="Pods not starting">
    ...
    ```bash
    # Get pod status
    kubectl get pods -n bchatsign
    
    # Describe pod
    kubectl describe pod <pod-name> -n bchatsign
    ```
  ...
  <Accordion title="Database connection errors">
    ...
    ```bash
    kubectl run postgres-test --rm -it --image=postgres:15-alpine -n bchatsign -- psql "postgresql://user:password@host:5432/database" -c "SELECT 1"
    ```
  </Accordion>
  <Accordion title="Ingress not working">
    ...
    ```bash
    # Get Ingress status
    kubectl get ingress -n bchatsign
    
    # Describe Ingress
    kubectl describe ingress bchatsign -n bchatsign
    ```
  ...
  <Accordion title="Certificate errors">
    ...
    kubectl get secret bchatsign-signing-cert -n bchatsign -o yaml
    ```
    ...
    kubectl exec -it <pod-name> -n bchatsign -- ls -la /opt/bchatsign/
    ```
  </Accordion>
