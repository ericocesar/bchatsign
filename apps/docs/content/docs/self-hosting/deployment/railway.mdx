---
title: Railway
description: Deploy Bchatsign on Railway with automatic builds from Git, managed PostgreSQL, and built-in networking. Use the one-click template or manual setup.
---
...
<Callout type="warn">
  ...
  [GitHub Discussions](https://github.com/bchatsign/bchatsign/discussions) for help.
</Callout>
...
Once deployed, Railway provides a URL like `bchatsign-production.up.railway.app`.

Click the URL in the Railway dashboard to access your Bchatsign instance.
...
If you prefer more control over the setup, deploy Bchatsign manually:
...
- Fork or select the [Bchatsign repository](https://github.com/bchatsign/bchatsign)
...
Click on the Bchatsign service and go to the "Variables" tab. Add the following variables:
...
| `BCHATSIGN_DISABLE_TELEMETRY`     | Disable anonymous telemetry        | `false` |
...
Bchatsign runs database migrations automatically on startup.
...
- Click on your Bchatsign service
...
Railway provides several options for scaling your Bchatsign deployment:
...
<Steps>
<Step>
### Open your Bchatsign service

In the Railway dashboard, select the Bchatsign service.
...
In the Railway dashboard, select your Bchatsign service and open "Settings".
...
<Callout type="warn">
  Bchatsign stores sessions in the database, so horizontal scaling works without additional
...
Railway uses usage-based pricing. Estimated monthly costs for Bchatsign:
...
Bchatsign requires a signing certificate to sign documents. On Railway, you can:

### Option 1: Use Generated Certificate (Default)

Bchatsign generates a self-signed certificate automatically if none is provided. This works for testing but shows generic certificate information in signed documents.
...
In your Bchatsign service variables, set:
...
<a href="https://app.koyeb.com/deploy?type=git&repository=github.com/bchatsign/bchatsign&branch=main&name=bchatsign-app&builder=dockerfile&dockerfile=/docker/Dockerfile">
  <img
