...
description: Deploy Bchatsign on a Linux server without Docker using Node.js and systemd.
---
...
Navigate to the `bchatsign` folder and create a `.env` file:
...
NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@localhost:5432/bchatsign"
NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@localhost:5432/bchatsign"
NEXT_PRIVATE_SMTP_FROM_NAME="Bchatsign"
NEXT_PRIVATE_SMTP_FROM_ADDRESS="noreply@your-domain.com"
```

<Callout type="info">
  If you use a reverse proxy in front of Bchatsign, set `NEXT_PUBLIC_WEBAPP_URL` to the public URL
...
Create a service file to run Bchatsign as a background service on Linux:

```ini
# /etc/systemd/system/bchatsign.service

[Unit]
Description=Bchatsign
After=network.target

[Service]
Type=simple
User=www-data
WorkingDirectory=/var/www/bchatsign
EnvironmentFile=/var/www/bchatsign/.env
...
```bash
sudo systemctl enable bchatsign
sudo systemctl start bchatsign
```

## Reverse Proxy with Nginx

A minimal Nginx configuration for proxying to Bchatsign:
