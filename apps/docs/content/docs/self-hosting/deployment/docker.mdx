---
title: Docker
description: Run Bchatsign as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
The Bchatsign image is available on both DockerHub and GitHub Container Registry:
...
docker pull ghcr.io/bchatsign/bchatsign:latest
```
...
These variables must be set for Bchatsign to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where Bchatsign is accessible (e.g., `https://sign.example.com`) | -                             |
...
Bchatsign requires two database connection strings:
...
| `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`      | Path to signing certificate inside container                   | `/opt/bchatsign/cert.p12` |
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@db-host:5432/bchatsign" \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="Bchatsign" \
...
- **Volume mount** — mount a `.p12` file from the host into the container at `/opt/bchatsign/cert.p12` (shown above). This is the simplest approach for small to moderate deployments.
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@db-host:5432/bchatsign
...
Bchatsign provides health check endpoints for monitoring:
...
```bash
docker inspect --format='{{.State.Health.Status}}' bchatsign
```

## Updating the Container

To update Bchatsign to a new version:
...
The Bchatsign container is stateless. All persistent data is stored in the PostgreSQL database.
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check the container logs: `docker logs bchatsign`. Common causes: missing required environment
