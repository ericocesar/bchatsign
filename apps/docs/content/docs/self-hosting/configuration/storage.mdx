...
S3 storage is recommended for production deployments. Bchatsign supports AWS S3 and any S3-compatible storage service.
...
```bash
aws s3 mb s3://your-bchatsign-bucket --region us-east-1
```
...
[
  {
    ...
    "AllowedOrigins": ["https://your-bchatsign-domain.com"],
    "ExposeHeaders": ["ETag"]
  }
]
```
...
```bash
aws s3api put-bucket-cors --bucket your-bchatsign-bucket --cors-configuration file://cors.json
```
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::your-bchatsign-bucket/*"
    }
  ]
}
```
...
Using Docker Compose with Bchatsign:
...
```bash
NEXT_PUBLIC_UPLOAD_TRANSPORT=s3
NEXT_PRIVATE_UPLOAD_BUCKET=bchatsign
...
Bchatsign works with any S3-compatible storage service. Configure the endpoint and enable path-style URLs if required.
...
Azure Blob Storage is supported as a native transport (not S3-compatible). Bchatsign uses the official `@azure/storage-blob` SDK and signs SAS URLs with the Storage Account key for browser uploads and downloads.
...
az storage cors add \
  --services b \
  --methods GET PUT \
  --origins https://your-bchatsign-domain.com \
...
NEXT_PRIVATE_UPLOAD_AZURE_CONTAINER=bchatsign-documents
```
...
- An S3 bucket configured for Bchatsign
...
Bchatsign does not provide automatic migration between storage backends. Each document's storage location is recorded in the database.
...
During migration, Bchatsign can read from both backends. The `DocumentData.type` field indicates where each document is stored.
