...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your Bchatsign deployment.
---
...
Bchatsign requires **PostgreSQL 14 or later**. No other databases are supported.
...
```
postgresql://bchatsign:password@localhost:5432/bchatsign
```

**Remote server with SSL:**

```
postgresql://bchatsign:password@db.example.com:5432/bchatsign?sslmode=require
```
...
```
postgresql://bchatsign:p%40ss%23word@localhost:5432/bchatsign
```

## Environment Variables

Bchatsign uses two database connection variables:
...
```bash
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@host:5432/bchatsign
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@host:5432/bchatsign
```

### Automatic Detection

Bchatsign automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, Bchatsign will use them automatically.
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@pooler-host:6432/bchatsign?pgbouncer=true

# Direct connection (bypasses PgBouncer)
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@db-host:5432/bchatsign
```
...
Bchatsign uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:

```
postgresql://user:password@host:5432/bchatsign?connection_limit=10&pool_timeout=30
```
...
```
postgresql://user:password@host:5432/bchatsign?sslmode=require
```
...
```
postgresql://user:password@host:5432/bchatsign?sslmode=verify-full&sslrootcert=/path/to/ca.crt
```
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@host:5432/bchatsign?sslmode=verify-full&sslrootcert=/etc/ssl/certs/db-ca.crt" \
  bchatsign/bchatsign:latest
```

## Running Migrations

Database migrations update your schema when upgrading Bchatsign.

### Automatic Migrations

When running Bchatsign via Docker, migrations run automatically on container startup. No manual intervention is required.
...
<Accordions type="multiple">
  ...
  <Accordion title="Connection timeout during migration">
    ...
    ```bash
    NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@host:5432/bchatsign?connect_timeout=60"
    ```
  </Accordion>
</Accordions>
...
<Accordions type="multiple">
  ...
  <Accordion title="Neon">
    ...
    **Configuration:**
    ...
    ```bash
    # Pooled connection
    NEXT_PRIVATE_DATABASE_URL=postgresql://[user]:[password]@[endpoint]-pooler.region.aws.neon.tech/bchatsign?sslmode=require

    # Direct connection for migrations
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://[user]:[password]@[endpoint].region.aws.neon.tech/bchatsign?sslmode=require
    ```
  ...
  <Accordion title="AWS RDS">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://bchatsign:[password]@your-instance.region.rds.amazonaws.com:5432/bchatsign?sslmode=require
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://bchatsign:[password]@your-instance.region.rds.amazonaws.com:5432/bchatsign?sslmode=require
    ```
  ...
  <Accordion title="Google Cloud SQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://bchatsign:[password]@/bchatsign?host=/cloudsql/[project]:[region]:[instance]
    ```
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://bchatsign:[password]@[public-ip]:5432/bchatsign?sslmode=require
    ```
  ...
  <Accordion title="Azure Database for PostgreSQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://bchatsign@[server-name]:[password]@[server-name].postgres.database.azure.com:5432/bchatsign?sslmode=require
    ```
  ...
  <Accordion title="DigitalOcean Managed Databases">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://doadmin:[password]@[cluster-host]:25060/bchatsign?sslmode=require
    ```
...
```bash
pg_dump -h host -U user -d bchatsign -F c -f bchatsign_backup.dump
```

**Restore from backup:**

```bash
pg_restore -h host -U user -d bchatsign -c bchatsign_backup.dump
```
...
Key parameters for Bchatsign workloads:
...
Bchatsign includes necessary indexes by default. Additional indexes may help for:
...
<Accordions type="multiple">
  ...
  <Accordion title="Authentication failed">
    ...
    Solutions:
    - Reset password with `ALTER USER bchatsign WITH PASSWORD 'newpassword';`
  ...
  <Accordion title="Database does not exist">
    ...
    ```sql
    CREATE DATABASE bchatsign;
    GRANT ALL PRIVILEGES ON DATABASE bchatsign TO bchatsign;
    ```
  </Accordion>
  <Accordion title="Permission denied">
    ...
    ```sql
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO bchatsign;
    GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO bchatsign;
    ```
  </Accordion>
</Accordions>
