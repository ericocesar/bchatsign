...
description: Complete reference for all environment variables used to configure a self-hosted Bchatsign instance.
---

## Required Variables

These variables must be set for Bchatsign to function:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL of your Bchatsign instance (e.g., `https://sign.example.com`)                   |
...
Bchatsign requires PostgreSQL 14 or higher.
...
Bchatsign supports multiple email transports for sending notifications.
...
Bchatsign can store documents in the database or S3-compatible storage.
...
Bchatsign requires a certificate to digitally sign documents.
...
Bchatsign can use Google Vertex AI for recipient and field detection.
...
Bchatsign can accept `.docx` uploads by sending them to a [Gotenberg](https://gotenberg.dev) service that converts them to PDF. When `NEXT_PRIVATE_DOCUMENT_CONVERSION_URL` is unset, DOCX uploads are rejected and only PDFs are accepted.
...
Bchatsign supports multiple background job providers for processing emails, documents, webhooks, and scheduled tasks.
...
| `NEXT_PRIVATE_REDIS_PREFIX`        | No       | Key prefix for Redis queues (useful when sharing an instance) | `bchatsign` |
...
| Variable                      | Description                                  | Default |
| ----------------------------- | -------------------------------------------- | ------- |
| `BCHATSIGN_DISABLE_TELEMETRY` | Set to `true` to disable anonymous telemetry | `false` |

Telemetry also auto-disables when `NEXT_PRIVATE_BCHATSIGN_LICENSE_KEY` is configured.
...
These variables require an active [Enterprise Edition](/docs/policies/enterprise-edition) license. Obtain a license key from [license.bchatsign.com](https://license.bchatsign.com) and set it below to unlock enterprise features such as SSO, embed editor, and 21 CFR Part 11 compliance.

| Variable                             | Description                                      |
| ------------------------------------ | ------------------------------------------------ |
| `NEXT_PRIVATE_BCHATSIGN_LICENSE_KEY` | License key for enterprise features              |
...
# Database
NEXT_PRIVATE_DATABASE_URL="postgres://user:password@localhost:5432/bchatsign"
NEXT_PRIVATE_DIRECT_DATABASE_URL="postgres://user:password@localhost:5432/bchatsign"
