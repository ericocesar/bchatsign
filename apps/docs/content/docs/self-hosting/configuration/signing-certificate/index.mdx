...
description: Configure the X.509 signing certificate used for digital signatures in self-hosted Bchatsign.
---

import { Tab, Tabs } from 'fumadocs-ui/components/tabs';

When a document is completed in Bchatsign, it is digitally signed using an X.509 certificate. This cryptographic signature provides:
...
Self-hosted Bchatsign instances require a signing certificate. You can generate a self-signed certificate or purchase one from a Certificate Authority (CA).
...
For organisations requiring hardware-based key protection, Bchatsign supports Google Cloud HSM. This provides:
