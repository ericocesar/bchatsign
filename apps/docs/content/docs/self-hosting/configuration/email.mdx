...
Bchatsign supports four email transport methods:
...
SMTP is the most common email transport. Bchatsign supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for Bchatsign to use.
...
Bchatsign uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the Bchatsign logo
...
Configure Bchatsign to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="Bchatsign Test"
```
...
```bash
# Docker
docker logs bchatsign 2>&1 | grep -i email
...
For MailChannels, configure DKIM directly in Bchatsign using the environment variables described above.
