...
description: Configure Google Vertex AI so Bchatsign can detect recipients and fields automatically.
---
...
This guide covers how to enable the AI recipient and field detection features when you self-host Bchatsign.
...
- Bchatsign version that includes the AI detection feature and the corresponding database migration.
...
<Callout type="info">
  ...
  If you omit the location, Bchatsign uses `global`. Not all models are available in every region;
  if a model is unavailable, switch to a supported region.
</Callout>

## Deploy with the Published Container

- Use the official Bchatsign image (DockerHub or GHCR) and supply the Vertex env vars above.
...
## Enable the Feature in Bchatsign
