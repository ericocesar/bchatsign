...
description: Enable DOCX uploads on a self-hosted Bchatsign instance by running a Gotenberg sidecar that converts Word documents to PDF.
---
...
Bchatsign can accept `.docx` uploads in addition to PDFs. When a user uploads a Word document, the Bchatsign server sends it to a [Gotenberg](https://gotenberg.dev) service which uses LibreOffice to convert it to PDF. The converted PDF is what gets stored, signed, and downloaded. The original DOCX is discarded.
...
| Output format           | PDF                                                                  |
| Network requirement     | Bchatsign must reach the Gotenberg HTTP API                          |
...
- Network reachability from the Bchatsign container to the Gotenberg HTTP API.
- A version of Bchatsign that includes the document conversion feature.
...
For better fidelity, especially for non-Latin scripts, build a derived image that adds Microsoft Core Fonts and additional language fonts. The Bchatsign repository ships a reference Dockerfile at [`docker/development/Dockerfile.gotenberg`](https://github.com/bchatsign/bchatsign/blob/main/docker/development/Dockerfile.gotenberg) that you can use as a starting point:
...
```bash
docker build -t registry.example.com/bchatsign/gotenberg:8 \
  -f Dockerfile.gotenberg .
docker push registry.example.com/bchatsign/gotenberg:8
```
...
The Gotenberg service should run **alongside** your Bchatsign container, not exposed to the public internet. The conversion service has no built-in authorisation beyond HTTP Basic auth, so it should sit on a private network or behind your existing reverse proxy.

<Tabs items={['Docker Compose', 'Kubernetes', 'External Instance']}>
<Tab value="Docker Compose">

Add a `gotenberg` service to the `compose.yml` you use for Bchatsign:

```yaml
services:
  gotenberg:
    image: registry.example.com/bchatsign/gotenberg:8
...
Do **not** publish Gotenberg's port (`3000`) to the host. Bchatsign reaches it over the internal Docker network using the service name (`http://gotenberg:3000`).
...
Then reference the in-cluster URL from Bchatsign's environment:

```
NEXT_PRIVATE_DOCUMENT_CONVERSION_URL=http://gotenberg.bchatsign.svc.cluster.local:3000
NEXT_PRIVATE_DOCUMENT_CONVERSION_USERNAME=bchatsign
...
Bchatsign does not have to colocate with Gotenberg. You can point it at any reachable Gotenberg deployment: a managed instance, a shared internal service, or a Gotenberg-compatible API.
...
- Be reachable from the Bchatsign container with low enough latency that the 30 second per-request timeout is comfortable.
...
| `--api-timeout=500s`              | Server-side request ceiling. Bchatsign aborts at 30 s by default, so this is a safety net for very large documents.                                                                                                                                                  |
...
| `--pdfengines-disable-routes`     | Disables the PDF engines routes Bchatsign does not use. Shrinks the attack surface.                                                                                                                                                                                  |
| `--webhook-disable`               | Disables webhook callbacks. Bchatsign uses synchronous requests only.                                                                                                                                                                                                |

## Configure Bchatsign

Set the following environment variables on the Bchatsign container and restart it.
...
<Steps>
<Step>
### Restart the Bchatsign container
...
From a shell inside the Bchatsign container or another container on the same network:
...
In the Bchatsign web UI, open **Documents** and try uploading a small `.docx` file. The upload dropzone should accept it, and after a few seconds the editor should open with the converted PDF.
...
- **Disable unused routes.** `--pdfengines-disable-routes` and `--webhook-disable` reduce attack surface. Bchatsign only uses the LibreOffice convert route.
...
- **Rotate credentials.** Rotating the basic auth secret is a config change in both Gotenberg and Bchatsign, followed by a restart of each.
...
<Accordions type="multiple">
  <Accordion title="DOCX uploads are rejected with 'Only PDF and DOCX files are allowed'">
    The Bchatsign server does not see `NEXT_PRIVATE_DOCUMENT_CONVERSION_URL`. Check the value is set
    on the running container (`docker exec bchatsign printenv | grep DOCUMENT_CONVERSION`) and
    restart after changing it.
  </Accordion>

  <Accordion title="Uploads fail with 'Document conversion service is currently unavailable'">
    Bchatsign could not reach Gotenberg. Verify:

    - The URL in `NEXT_PRIVATE_DOCUMENT_CONVERSION_URL` is resolvable from the Bchatsign container
  ...
  <Accordion title="Form controls in the DOCX appear blank or missing">
    Bchatsign disables Gotenberg's `exportFormFields` flag during conversion. Word content controls
    (`<w:sdt>`) become static graphics in the output PDF, which prevents Bchatsign's later
    flattening step from making them invisible. This is intentional. Use Bchatsign fields
