...
description: Configure how Bchatsign processes background tasks like email delivery, document processing, and webhook dispatch.
---
...
Bchatsign processes background jobs for email delivery, document sealing, webhook dispatch, and scheduled maintenance tasks. Three providers are available:
...
[BullMQ](https://docs.bullmq.io/) is a Redis-backed job queue that runs inside the Bchatsign process. It provides higher throughput than the local provider, configurable concurrency, and a built-in dashboard for monitoring jobs.
...
| `NEXT_PRIVATE_REDIS_PREFIX`        | Key prefix for Redis queues (useful when sharing an instance)              | `bchatsign` |
...
Then set `NEXT_PRIVATE_REDIS_URL=redis://redis:6379` in your Bchatsign environment.
...
The local provider uses your PostgreSQL database as a job queue. Jobs are stored in the `BackgroundJob` table and processed via internal HTTP requests that Bchatsign sends to itself.
...
Background jobs in the local provider work by Bchatsign sending HTTP requests to itself. If your reverse proxy or network setup causes issues with the app reaching its own public URL, set the internal URL:
