...
description: Practical advice for running Bchatsign in production.
---
...
<Callout type="error">
  This is the single most common issue when self-hosting Bchatsign. Without a signing certificate,
  the application starts normally but document signing fails.
</Callout>

Bchatsign does **not** bundle a [signing certificate](/docs/self-hosting/configuration/signing-certificate) in the Docker image. You must provide one yourself using one of two methods:
...
```bash
docker run -d \
  -v /path/to/your/cert.p12:/opt/bchatsign/cert.p12:ro \
  -e NEXT_PRIVATE_SIGNING_PASSPHRASE="your-certificate-password" \
  bchatsign/bchatsign:latest
```

The default certificate path inside the container is `/opt/bchatsign/cert.p12`. You can change this with `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`.
...
After starting Bchatsign, check the certificate status endpoint:
...
Always set a password when creating your `.p12` certificate. When using OpenSSL, set the password during the export step and provide it to Bchatsign via `NEXT_PRIVATE_SIGNING_PASSPHRASE`.

---

## Use the Health Check Endpoints

Bchatsign exposes two endpoints for monitoring:
...
docker pull bchatsign/bchatsign:1.8.0

# Risky — may pull breaking changes
docker pull bchatsign/bchatsign:latest
```

Check the [GitHub releases](https://github.com/bchatsign/bchatsign/releases) page for available versions.
...
Background jobs work by Bchatsign sending HTTP requests to itself. If your reverse proxy or network setup causes issues with the app reaching its own public URL, set the internal URL:
...
NEXT_PRIVATE_UPLOAD_BUCKET=bchatsign-documents
...
Bchatsign uses a PostgreSQL-based job queue by default (`local` provider). No Redis or external message broker is required for basic deployments.
...
```bash
docker run -it -e HOST=:: bchatsign/bchatsign:latest npm run start
```
...
The Bchatsign container runs as a non-root user (UID 1001). If you mount files into the container (certificates, configuration), ensure they're readable:
