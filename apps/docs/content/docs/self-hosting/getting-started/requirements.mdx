---
title: Requirements
description: System requirements and prerequisites for self-hosting Bchatsign.
---
...
Bchatsign requires the following items and external services:
...
<Callout type="error">
  Bchatsign does not ship with a signing certificate. Without one, the application starts normally
...
Bchatsign uses PostgreSQL for all data storage including documents, users, and audit logs. You cannot use MySQL, SQLite, or other databases.

Bchatsign requires two connection strings:
...
Bchatsign sends transactional emails for:
...
For production deployments, place Bchatsign behind a reverse proxy such as:
...
The reverse proxy handles SSL/TLS termination and forwards requests to Bchatsign on port 3000.
...
By default, Bchatsign stores documents in the PostgreSQL database. For production deployments with significant document volume, use S3-compatible storage:
...
Bchatsign processes background jobs (email delivery, document processing) using a PostgreSQL-based queue by default. No additional services are required: the job queue is built into the application and uses your existing database.

For production deployments that need higher throughput or more reliable job processing, Bchatsign supports [BullMQ](https://docs.bullmq.io/) as an alternative provider. BullMQ requires a **Redis** instance (v6.2+). Set `NEXT_PRIVATE_JOBS_PROVIDER=bullmq` and configure `NEXT_PRIVATE_REDIS_URL`.
...
- [ ] Domain name for your Bchatsign instance
...
- [Quick Start](/docs/self-hosting/getting-started/quick-start) - Deploy Bchatsign with Docker in 5 minutes
