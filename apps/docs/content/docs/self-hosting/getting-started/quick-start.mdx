---
title: Quick Start
description: Get Bchatsign running locally in minutes with Docker Compose.
---
...
docker compose -f docker/development/compose.yml exec database \
  psql -U bchatsign -d bchatsign -c "UPDATE \"User\" SET roles = '{ADMIN}' WHERE email = 'your@email.com';"
```
...
| ----------- | -------------------------------- | ----- |
| `bchatsign` | Main application                 | 3000  |
...
<Accordions type="multiple">
  ...
  <Accordion title="Container fails to start">
    Check the logs: `docker compose -f docker/development/compose.yml logs bchatsign`. Common
