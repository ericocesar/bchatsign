---
title: Upgrades
description: Upgrade your self-hosted Bchatsign installation to get the latest features, security patches, and bug fixes.
---
...
Bchatsign follows [Semantic Versioning](https://semver.org/):
...
<Callout type="info">
  Review the [release notes](https://github.com/bchatsign/bchatsign/releases) before upgrading to
...
Check your running Bchatsign version:

<Tabs items={['Docker', 'Docker Compose', 'Kubernetes']}>
<Tab value="Docker">

```bash
docker inspect bchatsign --format='{{.Config.Image}}'
```
...
```bash
kubectl get deployment bchatsign -n bchatsign -o jsonpath='{.spec.template.spec.containers[0].image}'
```
...
```bash
curl -s https://api.github.com/repos/bchatsign/bchatsign/releases/latest | grep tag_name
```

Or visit the [releases page](https://github.com/bchatsign/bchatsign/releases).
...
```bash
curl -s https://hub.docker.com/v2/repositories/bchatsign/bchatsign/tags | jq -r '.results[].name'
```
...
```bash
curl -s https://api.github.com/orgs/bchatsign/packages/container/bchatsign/versions | jq -r '.[].metadata.container.tags[]'
```
...
```bash
docker compose exec database pg_dump -U bchatsign bchatsign > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
kubectl exec -n bchatsign deploy/postgres -- pg_dump -U bchatsign bchatsign > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
pg_dump "postgresql://user:password@host:5432/bchatsign" > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
kubectl get secret bchatsign-secrets -n bchatsign -o yaml > secrets-backup.yaml
kubectl get configmap bchatsign-config -n bchatsign -o yaml > configmap-backup.yaml
```
...
```bash
docker pull bchatsign/bchatsign:1.6.0
```
...
docker run -d \
  ...
  -p 3000:3000 \
  --env-file .env \
  -v /path/to/cert.p12:/opt/bchatsign/cert.p12:ro \
  bchatsign/bchatsign:1.6.0
```
...
services:
  bchatsign:
    image: bchatsign/bchatsign:1.6.0
```
...
services:
  bchatsign:
    image: bchatsign/bchatsign:${BCHATSIGN_VERSION:-latest}
```
...
```bash
docker compose ps
docker compose logs -f bchatsign
```
...
```bash
kubectl set image deployment/bchatsign \
  bchatsign=bchatsign/bchatsign:1.6.0 \
  -n bchatsign
```
...
```bash
kubectl rollout status deployment/bchatsign -n bchatsign
```

Watch pods transition:

```bash
kubectl get pods -n bchatsign -w
```
...
```bash
kubectl get pods -n bchatsign -o wide
```

Test the health endpoint:

```bash
kubectl port-forward svc/bchatsign 3000:80 -n bchatsign &
curl http://localhost:3000/api/health
```
...
Bchatsign runs database migrations automatically when the container starts. No manual intervention is required.
...
```bash
docker logs bchatsign 2>&1 | grep -i migration
```
...
```bash
kubectl logs -l app.kubernetes.io/name=bchatsign -n bchatsign | grep -i migration
```
...
# Run migrations only
docker run --rm \
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@host:5432/bchatsign" \
  bchatsign/bchatsign:1.6.0 \
...
Read the [release notes](https://github.com/bchatsign/bchatsign/releases) carefully.
...
curl -O https://raw.githubusercontent.com/bchatsign/bchatsign/main/.env.example
...
kubectl rollout history deployment/bchatsign -n bchatsign

# Rollback to previous version
kubectl rollout undo deployment/bchatsign -n bchatsign

# Or rollback to a specific revision
kubectl rollout undo deployment/bchatsign -n bchatsign --to-revision=2
```
...
```bash
docker compose exec -T database psql -U bchatsign bchatsign < backup-20240115-120000.sql
```
...
```bash
psql "postgresql://user:password@host:5432/bchatsign" < backup-20240115-120000.sql
```
...
<Accordions type="multiple">
  <Accordion title="Container fails to start after upgrade">
    **Check logs:** `docker logs bchatsign`
  ...
  <Accordion title="Migration errors">
    ...
    **Test connectivity:**
    ...
    ```bash
    docker run --rm postgres:15 psql "postgresql://user:password@host:5432/bchatsign" -c "SELECT 1"
    ```
  ...
  <Accordion title="Application errors after upgrade">
    **If the application starts but behaves unexpectedly:**
    ...
    ```bash
    docker logs bchatsign 2>&1 | grep -i -E "(warn|error)"
    ```
  </Accordion>
