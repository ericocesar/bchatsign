...
description: Diagnose and resolve common issues with your self-hosted Bchatsign instance, organized by component.
---
...
```bash
kubectl logs -l app.kubernetes.io/name=bchatsign -n bchatsign -f
```
...
```bash
# Docker - find errors
docker logs bchatsign 2>&1 | grep -i error

# Docker - find warnings
docker logs bchatsign 2>&1 | grep -i warn

# Show last 100 lines
docker logs --tail 100 bchatsign
```

### Health Checks

Bchatsign exposes a health endpoint to verify the application is running:
...
```bash
docker exec bchatsign printenv | grep NEXT
```
...
```bash
docker compose exec bchatsign printenv | grep NEXT
```
...
```bash
docker inspect bchatsign --format='{{.State.ExitCode}}'
```
...
```bash
# Docker
docker stats bchatsign

# Kubernetes
kubectl top pod -n bchatsign
```
...
<Steps>
  ...
  <Step>
    Change Bchatsign's port mapping: `-p 3001:3000`
  </Step>
</Steps>
...
```
Error: password authentication failed for user "bchatsign"
```
...
```sql
ALTER USER bchatsign WITH PASSWORD 'newpassword';
```
...
```
Error: database "bchatsign" does not exist
```
...
```
Error: too many connections for role "bchatsign"
```
...
```
postgresql://user:password@host:5432/bchatsign?connection_limit=5
```
...
```bash
docker exec bchatsign printenv | grep SMTP
```
...
```bash
docker exec bchatsign nc -zv smtp.example.com 587
```
...
```bash
docker exec bchatsign printenv | grep SIGNING
```
...
```bash
docker exec bchatsign ls -la /opt/bchatsign/cert.p12
```
...
```
Error: EACCES: permission denied, open '/opt/bchatsign/cert.p12'
```
...
```
  ...
  <Step>
    Update the certificate file and restart Bchatsign
  </Step>
</Steps>
...
```bash
docker exec bchatsign cat /etc/resolv.conf
```
...
- **[GitHub Discussions](https://github.com/bchatsign/bchatsign/discussions)**
- **[Discord](https://documen.so/discord)**: Join the Bchatsign community Discord
- **[GitHub Issues](https://github.com/bchatsign/bchatsign/issues)**: For confirmed bugs, open an issue
...
For security vulnerabilities, do not open a public issue. Contact security@bchatsign.com directly.
...
<Accordions type="multiple">
  <Accordion title="Check application logs for errors">
    Run `docker logs bchatsign` or `docker compose logs bchatsign` (or `kubectl logs` for
