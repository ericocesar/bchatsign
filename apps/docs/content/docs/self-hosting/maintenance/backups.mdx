---
title: Backups
description: Backup strategies for Bchatsign data.
---
...
Bchatsign stores data in three locations:
...
```bash
pg_dump -h localhost -U bchatsign -d bchatsign -F c -f bchatsign_backup.dump
```
...
```bash
docker compose exec database pg_dump -U bchatsign -F c bchatsign > bchatsign_backup.dump
```
...
```bash
pg_dump -h localhost -U bchatsign -d bchatsign -F c \
  -f "bchatsign_$(date +%Y%m%d_%H%M%S).dump"
```
...
```bash
pg_dump -h localhost -U bchatsign -d bchatsign -F p -f bchatsign_backup.sql
```
...
```bash
pg_dump -h localhost -U bchatsign -d bchatsign | gzip > bchatsign_backup.sql.gz
```
...
```bash
cp /opt/bchatsign/cert.p12 /backup/bchatsign/cert.p12
```
...
```bash
echo "$NEXT_PRIVATE_SIGNING_LOCAL_FILE_CONTENTS" | base64 -d > /backup/bchatsign/cert.p12
```
...
```bash
aws s3 sync s3://your-bchatsign-bucket s3://your-backup-bucket
# or
aws s3 sync s3://your-bchatsign-bucket /backup/bchatsign/documents/
```
...
```bash
aws s3 sync s3://your-bucket /backup/bchatsign/documents/ \
...
Create a script to back up all Bchatsign components:
...
S3_BUCKET="your-bchatsign-bucket"

# Create backup directory
mkdir -p "$BACKUP_DIR/$TIMESTAMP"

echo "Starting Bchatsign backup: $TIMESTAMP"
...
if [ -f "/opt/bchatsign/cert.p12" ]; then
  echo "Backing up certificate..."
  cp /opt/bchatsign/cert.p12 "$BACKUP_DIR/$TIMESTAMP/cert.p12"
fi
...
tar -czf "$BACKUP_DIR/bchatsign_$TIMESTAMP.tar.gz" -C "$BACKUP_DIR" "$TIMESTAMP"
rm -rf "$BACKUP_DIR/$TIMESTAMP"

# Calculate checksum
sha256sum "$BACKUP_DIR/bchatsign_$TIMESTAMP.tar.gz" > "$BACKUP_DIR/bchatsign_$TIMESTAMP.sha256"

# Remove old backups
echo "Removing backups older than $RETENTION_DAYS days..."
find "$BACKUP_DIR" -name "bchatsign_*.tar.gz" -mtime +$RETENTION_DAYS -delete
find "$BACKUP_DIR" -name "bchatsign_*.sha256" -mtime +$RETENTION_DAYS -delete

echo "Backup completed: bchatsign_$TIMESTAMP.tar.gz"
```
...
```bash
#!/bin/bash
# bchatsign-backup-docker.sh

set -e

BACKUP_DIR="/backup/bchatsign"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
COMPOSE_PROJECT="bchatsign-production"
...
docker compose -p "$COMPOSE_PROJECT" exec -T database \
  pg_dump -U bchatsign -F c bchatsign > "$BACKUP_DIR/database_$TIMESTAMP.dump"

# Certificate backup (copy from container if needed)
docker compose -p "$COMPOSE_PROJECT" cp \
  bchatsign:/opt/bchatsign/cert.p12 "$BACKUP_DIR/cert_$TIMESTAMP.p12" 2>/dev/null || true
...
```bash
chmod +x /usr/local/bin/bchatsign-backup.sh
```
...
0 2 * * * /usr/local/bin/bchatsign-backup.sh >> /var/log/bchatsign-backup.log 2>&1

# Hourly backup (for high-activity instances)
0 * * * * /usr/local/bin/bchatsign-backup-hourly.sh >> /var/log/bchatsign-backup.log 2>&1
```

</Tab>
<Tab value="Systemd timer">

Create `/etc/systemd/system/bchatsign-backup.service`:

```ini
[Unit]
Description=Bchatsign Backup

[Service]
Type=oneshot
ExecStart=/usr/local/bin/bchatsign-backup.sh
User=root
```

Create `/etc/systemd/system/bchatsign-backup.timer`:

```ini
[Unit]
Description=Daily Bchatsign Backup
...
systemctl enable --now bchatsign-backup.timer
```
...
```bash
pg_restore -h localhost -U bchatsign -d bchatsign -c bchatsign_backup.dump
```

Docker Compose:

```bash
docker compose exec -T database pg_restore -U bchatsign -d bchatsign -c < bchatsign_backup.dump
```
...
```bash
psql -h localhost -U bchatsign -d bchatsign < bchatsign_backup.sql
```

For compressed backups:

```bash
gunzip -c bchatsign_backup.sql.gz | psql -h localhost -U bchatsign -d bchatsign
```
...
```bash
cp /backup/bchatsign/cert.p12 /opt/bchatsign/cert.p12
chmod 644 /opt/bchatsign/cert.p12
chown 1001:1001 /opt/bchatsign/cert.p12
```
...
```bash
aws s3 sync /backup/bchatsign/documents/ s3://your-bchatsign-bucket
```
...
```bash
docker compose exec -T database pg_restore -U bchatsign -d bchatsign -c < backup.dump
```
...
```bash
docker compose cp cert.p12 bchatsign:/opt/bchatsign/cert.p12
```
...
```bash
docker compose start bchatsign
```
...
BACKUP_FILE="$1"
TEST_DB="bchatsign_restore_test"
...
```bash
# Check checksum
sha256sum -c bchatsign_backup.sha256

# Verify pg_dump format
pg_restore --list bchatsign_backup.dump > /dev/null && echo "Backup is valid"
```
...
```bash
aws s3 cp bchatsign_backup.tar.gz s3://your-backup-bucket/bchatsign/
```
...
```bash
# Encrypt
gpg --symmetric --cipher-algo AES256 bchatsign_backup.tar.gz

# Decrypt
gpg --decrypt bchatsign_backup.tar.gz.gpg > bchatsign_backup.tar.gz
```
