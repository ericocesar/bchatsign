...
When someone uses a direct link, Bchatsign creates a new document from the template with that person as the signer.
...
curl -X POST "https://app.bchatsign.com/api/v2/template/direct/create" \
...
// URL: https://app.bchatsign.com/d/{token}
````
</Tab>
</Tabs>
...
```
https://app.bchatsign.com/d/{token}
```
...
```html
<iframe
  src="https://app.bchatsign.com/embed/direct/abc123xyz"
  width="100%"
  height="800"
  frameborder="0"
...
function DocumentSigner({ token }: { token: string }) {
  return (
    <div className="h-[800px] w-full">
      <iframe
        src={`https://app.bchatsign.com/embed/direct/${token}`}
...
function redirectToSigning(token: string) {
  window.location.href = `https://app.bchatsign.com/d/${token}`;
}
```
...
```
https://app.bchatsign.com/d/abc123xyz?externalId=order-12345
```
...
function getSigningUrl(token: string, orderId: string) {
  ...
  return `https://app.bchatsign.com/d/${token}?${params.toString()}`;
}
...
```
https://app.bchatsign.com/embed/direct/abc123xyz?externalId=order-12345
```
...
| Account   | Signer must be logged into a Bchatsign account |
...
<Tab value="curl">
```bash
curl -X POST "https://app.bchatsign.com/api/v2/template/direct/toggle" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123,
...
```bash
curl -X POST "https://app.bchatsign.com/api/v2/template/direct/delete" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123
...
Enterprise plans support white-label embedding with the "Powered by Bchatsign" badge removed. [Contact sales](mailto:support@bchatsign.com) for details.
...
function getEmbedUrl(token: string, orderId: string) {
  ...
  return `https://app.bchatsign.com/embed/direct/${token}?${params}`;
}
