...
Bchatsign offers two types of embedding:
...
```jsx
import { EmbedDirectTemplate } from '@bchatsign/embed-react';
...
```jsx
import { EmbedSignDocument } from '@bchatsign/embed-react';
...
<Steps>
<Step>
Navigate to your team's templates in Bchatsign
...
For example, `https://app.bchatsign.com/d/-WoSwWVT-fYOERS2MI37k` has the token `-WoSwWVT-fYOERS2MI37k`.
...
A [Web Components](https://developer.mozilla.org/en-US/docs/Web/API/Web_components) SDK (`@bchatsign/embed-webcomponent`) is also available for use outside of JavaScript frameworks. It works in any environment that supports custom elements.
...
| `host`                | `string`   | Bchatsign instance URL. Defaults to `https://app.bchatsign.com`. |
...
| `language`            | `string`   | Set the UI language. See [Supported Languages](https://github.com/bchatsign/bchatsign/tree/main/packages/lib/constants/locales.ts). |
