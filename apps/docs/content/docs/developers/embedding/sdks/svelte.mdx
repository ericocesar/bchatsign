---
title: Svelte
description: Embed Bchatsign signing in your Svelte application using the official SDK.
---
...
<Tabs items={['npm', 'yarn', 'pnpm']}>
  <Tab value="npm">``` npm install @bchatsign/embed-svelte ```</Tab>
  <Tab value="yarn">``` yarn add @bchatsign/embed-svelte ```</Tab>
  <Tab value="pnpm">``` pnpm add @bchatsign/embed-svelte ```</Tab>
</Tabs>
...
```svelte
<script lang="ts">
  import { EmbedDirectTemplate } from '@bchatsign/embed-svelte';
...
```svelte
<script lang="ts">
  import { EmbedSignDocument } from '@bchatsign/embed-svelte';
