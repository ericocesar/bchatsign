...
app.post('/webhooks/bchatsign', (req, res) => {
  ...
  const secret = req.headers['x-bchatsign-secret'] as string;
...
Register your endpoint in the Bchatsign dashboard:
...
<Steps>
  ...
  <Step>
    Enter your webhook URL (e.g. <code>https://your-app.com/webhooks/bchatsign</code>)
  </Step>
  <Step>
  ...
  <Step>
    Enter your webhook secret (the same value stored in <code>BCHATSIGN_WEBHOOK_SECRET</code>)
  </Step>
  <Step>
    Click **Create Webhook**
  </Step>
...
class BchatsignApiError extends Error {
  code?: string;
  statusCode: number;
  details?: Record<string, unknown>;
...
try {
  ...
  if (error instanceof BchatsignApiError) {
...
- [API Reference](https://openapi.bchatsign.com/) - Full endpoint documentation
