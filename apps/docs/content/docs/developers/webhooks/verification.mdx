...
Verifying webhook requests ensures that incoming payloads originate from Bchatsign and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How Bchatsign Signs Webhooks

When you configure a webhook with a secret, Bchatsign includes that secret in every webhook request via the `X-Bchatsign-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.

```bash
POST /your-webhook-endpoint HTTP/1.1
Host: your-server.com
Content-Type: application/json
X-Bchatsign-Secret: your_webhook_secret_here
...
| Header               | Description                                             |
| -------------------- | ------------------------------------------------------- |
| `X-Bchatsign-Secret` | The secret key you configured when creating the webhook |
...
<Steps>
  <Step>
    Extract the `X-Bchatsign-Secret` header from the incoming request
  </Step>
  <Step>
...
const WEBHOOK_SECRET = process.env.BCHATSIGN_WEBHOOK_SECRET;
...
app.post('/webhooks/bchatsign', (req, res) => {
  const receivedSecret = req.headers['x-bchatsign-secret'];
...
WEBHOOK_SECRET = os.environ.get('BCHATSIGN_WEBHOOK_SECRET')
...
@app.route('/webhooks/bchatsign', methods=['POST'])
def handle_webhook():
    received_secret = request.headers.get('X-Bchatsign-Secret', '')
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in Bchatsign |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in Bchatsign             |
...
3. Update the webhook configuration in Bchatsign
...
app.post('/webhooks/bchatsign', webhookLimiter, (req, res) => {
  // Handle webhook...
});
```
