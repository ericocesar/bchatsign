...
Webhooks are HTTP callbacks triggered by specific events in Bchatsign. When an event occurs (such as a document being signed), Bchatsign sends an HTTP POST request to your configured URL with details about the event.
...
Before configuring a webhook in Bchatsign, you need an endpoint that can receive HTTP POST requests. Here's a minimal example:
...
func main() {
	http.HandleFunc("/webhooks/bchatsign", webhookHandler)
...
<Callout type="warn">
  Always respond with a `200 OK` status within 30 seconds. Bchatsign will retry failed deliveries.
</Callout>

## Configuring Webhooks in Bchatsign via the Dashboard
...
Bchatsign provides a built-in testing feature to verify your webhook endpoint works correctly.
...
<Step>
Bchatsign will attempt to deliver the same payload again
</Step>
</Steps>

## Retry Policy

When a webhook delivery fails (non-2xx response or timeout), Bchatsign automatically retries with exponential backoff:
...
<Accordions type="multiple">
  ...
  <Accordion title="Verify signatures">
    Use a webhook secret and verify the `X-Bchatsign-Secret` header (see [Verification](/docs/developers/webhooks/verification)).
  </Accordion>
