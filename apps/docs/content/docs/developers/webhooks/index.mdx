...
1. You configure a webhook URL in Bchatsign
2. When an event occurs, Bchatsign sends an HTTP POST to your URL
3. Your application processes the event and responds with 200 OK

Bchatsign supports webhook events for the full document lifecycle (created, sent, opened, signed, completed, rejected, cancelled) as well as template events (created, updated, deleted, used).
