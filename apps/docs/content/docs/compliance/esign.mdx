...
## How Bchatsign Supports Compliance

Bchatsign provides features that support compliance with e-signature laws across jurisdictions:
...
Bchatsign maintains an audit trail for each document, recording:
...
Bchatsign supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:
