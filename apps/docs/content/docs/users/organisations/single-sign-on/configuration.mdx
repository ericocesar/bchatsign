...
Bchatsign supports any identity provider that implements the OpenID Connect (OIDC) protocol. Common providers include:
...
- **Redirect URI**: The value from the Bchatsign SSO settings page
...
Back in Bchatsign, enter the following:
...
Bchatsign verifies their identity with your provider.
...
- Cannot set a Bchatsign password (they must use SSO)
...
<Callout type="info">
  SSO-only enforcement is not currently available. Users who have an existing Bchatsign account with a password can still sign in with their original method in addition to SSO.
</Callout>
...
<Accordions type="multiple">
  ...
  - Copy the redirect URI exactly from Bchatsign's SSO settings
  ...
  <Accordion title="User already exists">
    If a user already has a Bchatsign account with the same email, their existing account will be linked to your team and they can sign in through your SSO portal going forward.
  </Accordion>
</Accordions>
...
To reduce friction for your users, create a custom subdomain that redirects to your SSO portal link. For example, `bchatsign.your-organisation.com` can redirect to the portal URL shown in your SSO settings.
