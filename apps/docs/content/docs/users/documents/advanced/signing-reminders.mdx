...
Signing reminders automatically email recipients who have not completed their signing. You configure when the first reminder goes out and how often it repeats — Bchatsign handles the rest until the recipient signs or the document leaves a pending state.
...
- **Enabled** — Bchatsign sends reminders on the schedule you configure below
...
1. Bchatsign sends the reminder email — same template as the original signing request, but the subject and preview are prefixed with **"Reminder:"**.
...
- The document uses **manual link distribution** — Bchatsign never emails recipients for these documents
