...
description: Learn how to use placeholder text in your PDFs for automatic field placement in Bchatsign.
---
...
Bchatsign can automatically detect placeholder text in your PDF documents and create fields at those locations. This allows you to prepare documents in your preferred editing tool (Word, Google Docs, etc.) with placeholders that become signature fields when uploaded.

## How It Works

When you upload a PDF, Bchatsign scans for text matching the placeholder pattern `{{...}}`. Each placeholder can specify:
...
When you upload a PDF with placeholders, Bchatsign will:
